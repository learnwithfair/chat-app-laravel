...
export function useChat() {
    ...
    const handleNewMessage = (incomingMsg) => {
        ...
        if (activeConversation.value?.id === conversationId) {
            ...
            // If message is not mine, add to unseen set
            if (!formatted.isMine) {
                unseenMessageIds.add(formatted.id);
            }

            nextTick(() => {
                scrollToBottom();
                // Re-setup observer to include new message
    ...
    const handleMessageDelivered = (statusData) => {
        console.log('📨 handleMessageDelivered:', statusData);

        const msg = messages.value.find(m => m.id === statusData.message_id);
        if (msg && msg.isMine && msg.status === 'sent') {
            msg.status = 'delivered';
            console.log('✅ Message status updated to delivered:', statusData.message_id);
        }

        // Update in cache
        const cached = messageCache.get(statusData.conversation_id);
        if (cached) {
            const cachedMsg = cached.messages.find(m => m.id === statusData.message_id);
            if (cachedMsg && cachedMsg.isMine && cachedMsg.status === 'sent') {
                cachedMsg.status = 'delivered';
            }
        }
    };
    ...
    // FIX: Enhanced handleMessageSeen with real-time update
    const handleMessageSeen = (statusData) => {
        console.log('👁️ handleMessageSeen:', statusData);
        ...
        if (msg && msg.isMine) {
            ...
            if (statusData.user) {
                ...
                if (!msg.seenBy.some(u => u.id === statusData.user.id)) {
                    ...
                    console.log(`✅ ${statusData.user.name} seen message ${statusData.message_id}`);
                }
            }
        }
        ...
        if (cached) {
            const cachedMsg = cached.messages.find(m => m.id === statusData.message_id);
            if (cachedMsg && cachedMsg.isMine) {
                if (cachedMsg.status !== 'seen') {
                    cachedMsg.status = 'seen';
                }

                if (statusData.user) {
                    if (!cachedMsg.seenBy) cachedMsg.seenBy = [];
                    if (!cachedMsg.seenBy.some(u => u.id === statusData.user.id)) {
                        cachedMsg.seenBy.push({
                            id: statusData.user.id,
                            name: statusData.user.name,
                            avatar: statusData.user.avatar_path || generateAvatar(statusData.user.name),
                            seenAt: formatTime(statusData.created_at || new Date())
    ...
    // FIX: Don't invalidate cache when updating existing conversation
    const addOrUpdateConversation = async (convData) => {
        ...
        if (existing) {
            ...
            conversationCache.value.unshift(formatted);
    ...
    const UpdateConversation = (convData) => {
        ...
        const cachedConv = conversationCache.value.find(c => c.id === convData.id);
        if (cachedConv) {
            cachedConv.isMuted = false;
        }
    };
    ...
    const removeConversation = (conversationId) => {
        ...
        conversationCache.value = conversationCache.value.filter(c => c.id !== conversationId);
    ...
    const updateConversationInfo = (convData) => {
        ...
        // Update cache as well
        const cacheIndex = conversationCache.value.findIndex(c => c.id === convData.id);
        if (cacheIndex !== -1) {
            conversationCache.value[cacheIndex] = {
                ...conversationCache.value[cacheIndex],
                name: convData.name || conversationCache.value[cacheIndex].name,
                avatar: convData.meta?.avatar || conversationCache.value[cacheIndex].avatar,
                settings: convData.meta?.group_setting || conversationCache.value[cacheIndex].settings
            };
        }
    ...
    const updateUnreadCount = (conversationId, count) => {
        ...
        // Update cache as well
        const cachedConv = conversationCache.value.find(c => c.id === conversationId);
        if (cachedConv) {
            cachedConv.unreadCount = count;
        }
    };
    ...
    const handleBlockedByOther = (conversationId) => {
        ...
        const cachedConv = conversationCache.value.find(c => c.id === conversationId);
        if (cachedConv) {
            cachedConv.isBlocked = true;
            cachedConv.blockedByThem = true;
            cachedConv.canSendMessage = false;
        }
    ...
    const handleUnblockedByOther = (conversationId) => {
        ...
        const cachedConv = conversationCache.value.find(c => c.id === conversationId);
        if (cachedConv) {
            cachedConv.isBlocked = false;
            cachedConv.blockedByThem = false;
            cachedConv.canSendMessage = true;
        }
    ...
    const startPrivateConversationAPI = async (userId) => {
        try {
            ...
            if (!existing) {
                ...
                conversationCache.value.unshift(newConv);
            }
    ...
    const updateConversationPreview = (conversationId, message) => {
        ...
        // Update cache
        const cacheIndex = conversationCache.value.findIndex(c => c.id === conversationId);
        if (cacheIndex !== -1) {
            conversationCache.value[cacheIndex] = {
                ...conversationCache.value[cacheIndex],
                lastMessage: buildLastMessagePreview(message),
        ...
        // Move in cache too
        if (cacheIndex !== -1) {
            const [movedCacheConv] = conversationCache.value.splice(cacheIndex, 1);
            conversationCache.value.unshift(movedCacheConv);
        }
    };
    ...
    const selectConversation = async (conversation) => {
        ...
        // Initialize unseen messages and setup observer
        initializeUnseenMessages();

        nextTick(() => {
            scrollToBottom();
            setupMessageObserver(); // Setup auto-seen observer
        });
    };
    ...
    const createGroup = async ({ name, description, type, members }) => {
        try {
            ...
            conversationCache.value.unshift(conversations.value[0]);
    ...
    const leaveGroup = async () => {
        ...
        try {
            ...
            conversationCache.value = conversationCache.value.filter(c => c.id !== activeConversation.value.id);
            activeConversation.value = null;
    ...
    const handleToggleBlock = async (conversationId) => {
        try {
            ...
            const cachedConv = conversationCache.value.find(c => c.id === conversationId);
            if (cachedConv) {
                cachedConv.isBlocked = conv.isBlocked;
                cachedConv.blockedByMe = conv.blockedByMe;
                cachedConv.blockedByThem = conv.blockedByThem;
            }
    ...
    const handleMuteAction = async (minutes) => {
        try {
            ...
            // Update cache
            const cachedConv = conversationCache.value.find(c => c.id === activeConversation.value.id);
            if (cachedConv) {
                cachedConv.isMuted = activeConversation.value.isMuted;
                cachedConv.mutedUntil = activeConversation.value.mutedUntil;
            }
    ...
    const setupMessageObserver = () => {
        ...
        // Only setup if we have an active conversation
        if (!activeConversation.value || !messageContainer.value) {
            return;
        }
            ...
            (entries) => {
                ...
                entries.forEach((entry) => {
                    if (entry.isIntersecting) {
                        ...
                        // Only track messages that are NOT mine and haven't been marked as seen yet
                        if (!isMine && unseenMessageIds.has(messageId)) {
                            visibleMessageIds.push(messageId);
                            unseenMessageIds.delete(messageId);
                        }
                    }
                });

                // If we have visible unseen messages, mark them as seen
                if (visibleMessageIds.length > 0) {
                    markSpecificMessagesAsSeen(visibleMessageIds);
                }
            },
            {
    ...
    const markSpecificMessagesAsSeen = async (messageIds) => {
        if (!activeConversation.value || messageIds.length === 0) return;

        try {
            // Call backend to mark as seen
            await axios.post(`${API_BASE}/messages/mark-seen`, {
                conversation_id: activeConversation.value.id,
                message_ids: messageIds
            });

            console.log(`✅ Marked ${messageIds.length} messages as seen:`, messageIds);
            ...
            messageIds.forEach(msgId => {
                ...
                const cached = messageCache.get(activeConversation.value.id);
                if (cached) {
                    const cachedMsg = cached.messages.find(m => m.id === msgId);
                    if (cachedMsg) {
                        cachedMsg.status = 'seen';
                    }
                }
    ...
    const initializeUnseenMessages = () => {
        // Clear previous unseen messages
        unseenMessageIds.clear();

        // Add all messages that are not mine to the unseen set
        messages.value.forEach(msg => {
            if (!msg.isMine && msg.status !== 'seen') {
                unseenMessageIds.add(msg.id);
            }
        });

        console.log(`📊 Initialized ${unseenMessageIds.size} unseen messages`);
    };
