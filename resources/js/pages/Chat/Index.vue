<template>
  <div class="flex h-screen bg-gray-100 overflow-hidden">
    ...
    <ConversationList :conversations="filteredConversations" :active-conversation="activeConversation"
      :active-tab="activeTab" :search-query="searchQuery" :online-users="onlineUsers"
      :conversation-pagination="conversationPagination" @select="selectConversation" @update-tab="activeTab = $event"
      @update-search="searchQuery = $event" @create-group="openCreateGroupModal" @start-chat="startPrivateChat"
      @load-more="loadMoreConversations" :show-start-chat-modal="modals.startChat" :start-chat-users="startChatUsers"
      :start-chat-loading="startChatLoading" :start-chat-pagination="startChatPagination"
      @open-start-chat-modal="openStartChatModal" @close-start-chat-modal="modals.startChat = false"
      @search-start-chat-users="searchStartChatUsers" @load-more-start-chat-users="loadMoreStartChatUsers"
      @select-start-chat-user="handleStartChatUserSelect" />

    <!-- Main Chat Area -->
    <div v-if="activeConversation" :class="[
      ...
      <MessageList ref="messageListRef" :messages="messages" :is-group="activeConversation.type === 'group'"
        :search-query="messageSearchQuery" :highlighted-message-id="highlightedMessageId" :typing-users="getTypingUsers"
        :message-pagination="messagePagination" :conversation="activeConversation" @reply="replyToMessage"
        @edit="editMessage" @forward="forwardMessage" @delete="showDeleteMenu" @show-reactions="openReactionModal"
        @show-details="openMessageDetails" @show-seen-by="showSeenByModal" @add-reaction="handleAddReaction"
      ...
      <MessageInput v-model="newMessage" :is-blocked="activeConversation.isBlocked"
        :blocked-by-me="activeConversation.blockedByMe" :blocked-by-them="activeConversation.blockedByThem"
        :can-send-message="activeConversation.canSendMessage" :conversation-type="activeConversation.type"
        :is-editing="!!editingMessage" :conversation-id="activeConversation.id" @send="handleSendMessage()"
        @send-voice="handleSendVoice" @send-files="handleSendMessage"
    ...
    <ConversationInfo v-if="showRightPanel && activeConversation" :conversation="activeConversation"
      :group-members="groupMembers" :load-more-group-members="loadMoreGroupMembers || (() => { })"
      :group-members-pagination="groupMembersPagination" :active-tab="activeRightTab" @update-tab="handleTabChange"
      @add-member="openAddMemberModal" @make-admin="makeAdmin" @remove-admin="removeAdmin" @remove-member="removeMember"
      @leave-group="leaveGroup" @update-settings="updateGroupSettings" @trigger-search="triggerSearchFromRightPanel"
      @toggle-block="handleToggleBlock" @toggle-mute="handleToggleMute" @delete-conversation="handleDeleteConversation"
      @delete-group="handleDeleteGroup" @update-avatar="handleUpdateAvatar"
      @update-description="handleUpdateDescription" @update-name="handleUpdateName"
      @open-media-library="handleOpenMediaLibrary" />

    <!-- Modals -->
    <CreateGroupModal v-if="modals.createGroup" :available-users="availableUsers" :pagination="availableUsersPagination"
      @load-more="loadMoreAvailableUsers" @close="closeModal('createGroup')" @create="createGroup" />

    <MediaLibraryModal v-if="modals.mediaLibrary" :media="mediaLibrary.media" :audio="mediaLibrary.audio"
      :files="mediaLibrary.files" :links="mediaLibrary.links" :loading="mediaLibraryLoading"
      @close="closeModal('mediaLibrary')" />

    <AddMemberModal v-if="modals.addMember" :availableUsers="availableUsers"
      :currentGroupMembers="activeConversation.members" @close="closeModal('addMember')"
      @add-multiple="addMembersToGroup" />

    <ReactionModal v-if="modals.reaction" :message-id="reactionModalData.messageId"
      :reactions="reactionModalData.reactions" @close="closeReactionModal" @fetch-reactions="handleReactionFetch" />

    <SeenByModal v-if="modals.seenBy" :users="currentSeenBy" @close="closeModal('seenBy')" />

    <MessageDetailsModal v-if="modals.messageDetails" :message="selectedMessageDetails"
      @close="closeModal('messageDetails')" />

    <DeleteMessageModal v-if="modals.deleteMessage" :message="messageToDelete" @close="closeModal('deleteMessage')"
