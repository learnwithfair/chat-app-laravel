    ...
    <div @click.stop class="bg-white rounded-lg shadow-xl max-w-md w-full">
      ...
      <div class="p-4 max-h-96 overflow-y-auto">
        <div v-for="conversation in filteredConversations" :key="conversation.id"
          @click="toggleConversation(conversation.id)" :class="[
          ...
          <input type="checkbox" :checked="selectedConversations.includes(conversation.id)" class="mr-3" />
        ...
        <div v-if="filteredConversations.length === 0" class="text-center text-gray-400 py-6">
      ...
      <div class="p-4 border-t border-gray-200">
        <button @click="handleForward" :disabled="selectedConversations.length === 0" :class="[
...
const filteredConversations = computed(() =>
  props.conversations.filter(
    c => c.id !== props.currentConversationId
  )
);
...
<style scoped>button { cursor: pointer; }</style>
